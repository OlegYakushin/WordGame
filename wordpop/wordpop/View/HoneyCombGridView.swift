...
struct HoneyCombGridView<Content:View, Item>: View where Item: RandomAccessCollection {
    ...
    //scrolview varsa burasi genislik yayarliyor
    ...
    func setUpHoneyCombGrid()->[[Item.Element]]{
        ...
        items.forEach{item in
            ...
            //sonda kalan itemlere bakiyoruz
            if count == items.count{
                if let last = rows.last{
                    //onceki kayitlrda kalan varmi
                    //ayrica 4-3-4 paterinne uygunmu
                    if rows.count >= 2{
...
//width preference key scrollview varsa
